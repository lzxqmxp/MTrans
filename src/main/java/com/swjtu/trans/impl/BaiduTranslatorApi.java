...
public final class BaiduTranslatorApi extends AbstractTranslator {
    private static final String url = "https://fanyi-api.baidu.com/api/trans/vip/translate";

    public BaiduTranslatorApi(){
        super(url);
    }
    ...
    public void setFormData(LANG from, LANG to, String text) {
        ...
        formData.put("q", text);
        formData.put("appid", "20180917000207801");
    }

    @Override
    public String query() throws Exception {
        formData.put("salt",RandomUtil.randomNumbers(5));
        formData.put("sign", DigestUtil.md5Hex(formData.get("appid")+formData.get("q")+formData.get("salt")+"pxOxglXv5JvgizPeWypR"));
